...
export const FLEX_WALLET_ERROR = {
...
export const flexWalletErrors: AbiError[] = Object.entries(FLEX_WALLET_ERROR).map(
    ([name, { exitCode, message }]) => abiError(name, exitCode, message),
);

export const PRICE_XCHG_ERROR = {
...
export const priceXchgErrors: AbiError[] = Object.entries(PRICE_XCHG_ERROR).map(
    ([name, { exitCode, message }]) => abiError(name, exitCode, message),
);
